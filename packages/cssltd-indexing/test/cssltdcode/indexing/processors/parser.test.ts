...
const mockParseMarkdown = mock()
mock.module("../../../../src/tree-sitter/markdownParser", () => ({
  parseMarkdown: mockParseMarkdown,
}))

import { CodeParser, codeParser } from "../../../../src/indexing/processors/parser"
import { loadRequiredLanguageParsers } from "../../../../src/tree-sitter/languageParser"
import { parseMarkdown } from "../../../../src/tree-sitter/markdownParser"
...
describe("CodeParser", () => {
  ...
  // Create a shared temp directory for tests that need real files
  beforeEach(async () => {
    tempDir = await mkdtemp(join(tmpdir(), "parser-test-"))

    mockLoadParsers.mockReset()
    mockParseMarkdown.mockReset()
    mockLanguageParser.js.parser.parse.mockReset()
    ...
    mockLoadParsers.mockResolvedValue(mockLanguageParser as any)
  })

  describe("parseFile", () => {
    test("should return empty array for unsupported extensions", async () => {
    ...
    test("should read file when no content is provided", async () => {
      ...
      const tempFile = join(tempDir, "test-read.js")
    ...
    test("should handle file read errors gracefully", async () => {
      const result = await parser.parseFile("/tmp/nonexistent-parser-test-file.js")
      expect(result).toEqual([])
    })

    test("should use provided fileHash when available", async () => {
  ...
  describe("parseContent", () => {
    test("should wait for pending parser loads", async () => {
    ...
    test("should handle parser load errors", async () => {
      mockLoadParsers.mockRejectedValue(new Error("Load failed"))
      const content = `function fallbackTest() {
  const value = 123
  const text = "This content is long enough to trigger fallback chunking"
  return value + text.length
    ...
    test("should use fallback chunking when no parser is available", async () => {
      mockLoadParsers.mockResolvedValue({} as any)
      const content = `function missingParser() {
  const message = "Fallback should be used when parser is missing"
  return message.toUpperCase()
    ...
    test("should cache parser failures by extension", async () => {
      mockLoadParsers.mockRejectedValue(new Error("Load failed"))
      const content = `function cacheFailure() {
  const message = "Parser should fail once per extension and then reuse fallback"
  return message
}`

      const first = await parser["parseContent"]("first.js", content, "hash1")
      const second = await parser["parseContent"]("second.js", content, "hash2")

      expect(first.length).toBeGreaterThan(0)
      expect(second.length).toBeGreaterThan(0)
      expect(mockLoadParsers).toHaveBeenCalledTimes(1)
    })
  })

  describe("_performFallbackChunking", () => {
    test("should chunk content when no captures are found", async () => {
    ...
    test("should respect MIN_BLOCK_CHARS for fallback chunks", async () => {
      const shortContent = "short"
      const result = await parser["_performFallbackChunking"]("test.js", shortContent, "hash", new Set())
      expect(result).toEqual([])
    })

    test("should respect 50-character minimum threshold for all languages", async () => {
  ...
  describe("_chunkLeafNodeByLines", () => {
    test("should chunk leaf nodes by lines", async () => {
      const mockNode = {
  ...
  describe("_chunkTextByLines", () => {
    test("should handle oversized lines by splitting them", async () => {
    ...
    test("should re-balance chunks when remainder is too small", async () => {
  ...
  describe("singleton instance", () => {
    test("should maintain parser state across calls", async () => {
  ...
  describe("Markdown Support", () => {
    ...
    test("should generate unique segment hashes for each markdown block", async () => {
...
Each section should have its own unique hash based on its content.`
    ...
    test("should use fallback chunking for markdown files without headers", async () => {
...
Additional content to ensure we exceed the minimum block size requirements for proper indexing.`
    ...
    test("should chunk large markdown files with no headers", async () => {
    ...
    test("should enforce MIN_BLOCK_CHARS for all markdown sections", async () => {
...
The content is comprehensive enough to be included in the search results.`
    ...
    test("should chunk large markdown sections and generate unique hashes for each chunk", async () => {
    ...
    test("should handle markdown with very long single lines with chunking", async () => {
...
More normal content.`
    ...
    test("should preserve header information when chunking large sections", async () => {
...
${largeContent}`
    ...
    test("should apply chunking logic based on MAX_BLOCK_CHARS and re-balancing", async () => {
    ...
    test("should handle markdown content before the first header", async () => {
...
${headerContent}`
    ...
    test("should handle markdown content after the last header", async () => {
...
This ensures that trailing content in markdown files is properly captured and indexed.`
    ...
    test("should handle very long paragraphs with chunking", async () => {
...
This conclusion section contains multiple lines to ensure it exceeds 100 characters.`
      ...
      const h1Blocks = result.filter((r) => r.type === "markdown_header_h1" || r.type === "markdown_header_h1_segment")
      expect(h1Blocks.length).toBeGreaterThan(1)
    ...
    test("should continue processing after encountering a very long line", async () => {
...
Adding more content to ensure we meet minimum block requirements.`
    ...
    test("should handle multiple oversized lines in sequence", async () => {
...
This content verifies that processing continues after multiple oversized lines.`
  ...
  describe("Edge case: Single oversized line in markdown", () => {
    ...
    test("should properly chunk a markdown file with a single very long line", async () => {
    ...
    test("should handle markdown with headers followed by oversized lines", async () => {
      const parser = new CodeParser()
    ...
    test("should not chunk markdown files with lines under the threshold", async () => {
      const parser = new CodeParser()
    ...
    test("should return empty array for markdown content below MIN_BLOCK_CHARS threshold", async () => {
      const parser = new CodeParser()
