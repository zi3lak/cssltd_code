...
import { shouldUseFallbackChunking } from "../../../../src/indexing/shared/supported-extensions"
...
describe("CodeParser - VB.NET and Fallback Extensions Support", () => {
  ...
  test("should use fallback chunking for VB.NET files", async () => {
...
`.trim()
    ...
    if (!first) return
    expect(first.file_path).toBe("test.vb")
  })

  test("should handle large VB.NET files with proper chunking", async () => {
...
`
  ...
  test("should handle empty VB.NET files", async () => {
    const emptyContent = ""
  ...
  test("should handle small VB.NET files below minimum chunk size", async () => {
  ...
  test("should use fallback chunking for other configured fallback extensions", async () => {
...
This document captures the rollout steps for the indexing service.

1. Update the workspace settings.
2. Run the package test suite.
3. Restart the watcher after a successful deploy.
`,
    ],
    [
      "config.yaml",
...
`,
  ...
  ])("should use fallback chunking for %s", async (file, content) => {
...
describe("Fallback Extensions Configuration", () => {
  test("should correctly identify extensions that need fallback chunking", () => {
