...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@cssltdcode/acme")).toBe("@cssltdcode/acme")
    expect(Npm.sanitize("@cssltdcode/acme@1.0.0")).toBe("@cssltdcode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/cssltdcode/acme.git"
    const expected = win ? "acme@git+https_//github.com/cssltdcode/acme.git" : spec
