...
// test/preload.ts pins CSSLTD_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.CSSLTD_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.CSSLTD_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.CSSLTD_MODELS_PATH = undefined
  Flag.CSSLTD_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.CSSLTD_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.CSSLTD_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
...
describe("ModelsDev Service", () => {
  ...
  it.live("get() recovers from a corrupted cache file by fetching a fresh catalog", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          Flag.CSSLTD_DISABLE_MODELS_FETCH = false
        }),
        () =>
          provided(
            state,
  ...
  it.live("refresh(true) fetches via HttpClient and updates the cache", () =>
    Effect.gen(function* () {
      ...
      expect(final.calls[0].userAgent).toStartWith("cssltdcode/")
      expect(final.calls[0].userAgent).toEndWith(`/${Flag.CSSLTD_CLIENT}`)
    }),
  )
