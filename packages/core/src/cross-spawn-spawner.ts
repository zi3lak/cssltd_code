...
import { prepareCommand as prepareSandbox } from "@cssltdcode/sandbox" // cssltdcode_change
import { tap as tapStdio, tapped } from "./cssltdcode/stdio-tap" // cssltdcode_change - Bun drops buffered stdio on close
...
export const make = Effect.gen(function* () {
  ...
  const setupOutput = (
    ...
    let stdout = proc.stdout
      ? NodeStream.fromReadable({
          evaluate: () => tapped(proc, "stdout"), // cssltdcode_change - read the spawn-time tap
          onError: (cause) => toPlatformError("fromReadable(stdout)", toError(cause), command),
        })
      : Stream.empty
    let stderr = proc.stderr
      ? NodeStream.fromReadable({
          evaluate: () => tapped(proc, "stderr"), // cssltdcode_change - read the spawn-time tap
  ...
  const spawn = (command: ChildProcess.StandardCommand, opts: NodeChildProcess.SpawnOptions) =>
    Effect.callback<readonly [NodeChildProcess.ChildProcess, ExitSignal], PlatformError.PlatformError>((resume) => {
      ...
      tapStdio(proc) // cssltdcode_change - must run in the same tick as spawn
      let end = false
  ...
  const spawnCommand: (
    ...
    function* (command) {
      switch (command._tag) {
        case "StandardCommand": {
          const validation = SpawnValidation.take(command) // cssltdcode_change - retain target validation through preparation
          const dir = yield* cwd(command.options)
          // cssltdcode_change start - prepare agent-scoped commands through the selected sandbox backend
          ...
          // cssltdcode_change start - close target-swap races at the raw spawn boundary
          if (validation)
          ...
          const [proc, signal] = yield* Effect.acquireRelease(
            // cssltdcode_change start - spawn the prepared command and options
            spawn(target, {
              cwd: dir,
            ...
            Effect.fnUntraced(function* ([proc, signal]) {
              ...
              const kill = timeout(proc, command, target.options) // cssltdcode_change
              if (done) {
              ...
              // cssltdcode_change start - preserve kill options from the prepared command
