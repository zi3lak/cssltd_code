export function base64Encode(value: string) {
  const normalized = value.replace(/\\/g, "/") // cssltdcode_change - normalize Windows paths
  const bytes = new TextEncoder().encode(normalized) // cssltdcode_change - encode as UTF-8 bytes
