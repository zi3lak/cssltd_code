...
export const CSSLTD_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const CSSLTD_MODE_KEY = "cssltdcode.mode"

export const CssltdcodeKeymapProvider = KeymapProvider
export const useCssltdcodeKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type CssltdcodeModeStack = ReturnType<typeof createCssltdcodeModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, CssltdcodeModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createCssltdcodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(CSSLTD_MODE_KEY, CSSLTD_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(CSSLTD_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(CSSLTD_MODE_KEY, stack.at(-1)?.mode ?? CSSLTD_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? CSSLTD_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(CSSLTD_MODE_KEY, undefined)
...
export function useCssltdcodeModeStack() {
  return getCssltdcodeModeStack(useCssltdcodeKeymap())
}

export function getCssltdcodeModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Cssltd mode stack is not registered for this keymap") // cssltdcode_change
  return value
}
...
export function registerCssltdcodeKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createCssltdcodeModeStack(keymap)
