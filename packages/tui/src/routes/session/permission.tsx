...
import { normalizeUrls } from "@/cssltdcode/util/url"
import { MemoryPermissionRegistry } from "@/cssltdcode/cli/cmd/tui/routes/session/memory-permission"
// cssltdcode_change end
import { CSSLTD_BASE_MODE, useBindings, useCommandShortcut } from "../../keymap"
...
function EditBody(props: { request: PermissionRequest }) {
  ...
  const hunks = createMemo(() => splitDiffHunks(diff())) // cssltdcode_change
        ...
        {(() => {
          const info = () => {
            ...
            if (permission === "webfetch") {
              const url = normalizeUrls(typeof data.url === "string" ? data.url : "") // cssltdcode_change
              return {
                icon: "%",
                title: `WebFetch ${url}`,
                body: (
            ...
            const custom = MemoryPermissionRegistry.render(permission, props.request) // cssltdcode_change
            if (custom) return custom // cssltdcode_change
          ...
          const header = () => (
            <box flexDirection="column" gap={0}>
              ...
              {/* cssltdcode_change start - explain protected Cssltd configuration access */}
                ...
                <box paddingLeft={4} flexShrink={0}>
                  <text fg={theme.textMuted}>
                    {props.request.permission === "edit"
                      ...
                      : "Cssltd configuration access always requires approval"}
                  </text>
                </box>
              </Show>
              {/* cssltdcode_change end */}
          ...
          // cssltdcode_change start - hide "Always allow" for protected Cssltd configuration access
          ...
          const body = (
            <Prompt
              ...
              header={header()}
              body={current.body}
              /* cssltdcode_change */ options={options}
              escapeKey="reject"
              fullscreen
              onSelect={(option) => {
                if (option === "always") {
  ...
  return (
    ...
    >
      <box gap={1} paddingLeft={1} paddingRight={3} paddingTop={1} paddingBottom={1}>
        ...
        <box paddingLeft={1}>
          <text fg={theme.textMuted}>Tell Cssltd what to do differently</text>
        </box>
      </box>
      <box
