...
import { useNudge } from "@/cssltdcode/cli/cmd/tui/context/nudge" // cssltdcode_change
...
import type { AssistantMessage, FilePart, UserMessage } from "@cssltdcode/sdk/v2"
...
import { CssltdSessionTuiSync } from "@/cssltdcode/session/tui-sync"
import { slashMatches } from "@/cssltdcode/cli/cmd/command-display"
import * as AgentRequirements from "@/cssltdcode/cli/agent-requirements"
import { createCostAlertController } from "@/cssltdcode/cli/cmd/tui/cost-alert"
import { MemoryPrompt } from "@/cssltdcode/cli/cmd/tui/component/memory-prompt"
// cssltdcode_change end
import { CSSLTD_BASE_MODE, useBindings, useCommandShortcut, useLeaderActive, useCssltdcodeKeymap } from "../../keymap"
import { useTuiConfig } from "../../config"
// cssltdcode_change start - vim modal editing for the prompt
import { useVim, VimModeIndicator, vimToggleCommand } from "@/cssltdcode/cli/cmd/tui/component/prompt"
// cssltdcode_change end
...
export type PromptProps = {
  sessionID?: string
  directory?: string // cssltdcode_change
  visible?: boolean
  disabled?: boolean
  onSubmit?: () => void
...
export function Prompt(props: PromptProps) {
  ...
  const nudge = useNudge() // cssltdcode_change
  ...
  const [store, setStore] = createStore<{
    ...
    interrupt: number
    exitPress: number // cssltdcode_change - track double ctrl+c to exit
    placeholder: number
  }>({
  ...
  // cssltdcode_change start - sync local agent/model whenever newest user message changes
  ...
  createEffect(() => {
    ...
    if (!parts) return
    if (!CssltdSessionTuiSync.model({ role: msg.role, parts })) return
  ...
  const promptCommands = createMemo(() =>
    [
      ...
      // cssltdcode_change start - vim modal editing toggle (palette + /vim)
      vimToggleCommand({
      ...
      },
      {
        ...
        name: "workspace.set",
        category: "Session",
        enabled: Flag.CSSLTD_EXPERIMENTAL_WORKSPACES,
        slashName: "warp",
        run: () => {
          workspace.open()
        },
  ...
  useBindings(() => ({
    ...
    bindings: tuiConfig.keybinds.gather("prompt.palette", [
      ...
      "prompt.stash.pop",
      "prompt.stash.list",
      "prompt.vim.toggle", // cssltdcode_change
      "session.interrupt",
      "workspace.set",
      "session.move",
    ]),
  ...
  const ref: PromptRef = {
    ...
    reset() {
      ...
      vim.resetVim() // cssltdcode_change - return to insert mode after the prompt is cleared
    },
    submit() {
      void submit()
    },
  ...
  // cssltdcode_change start - close autocomplete while blocking overlays hide the prompt
  createEffect(() => {
    ...
    [
      {
        ...
        run: () => {
          ...
          setStore("extmarkToPartIndex", new Map())
          vim.resetVim() // cssltdcode_change
          dialog.clear()
        },
      },
      {
  ...
  // cssltdcode_change start - require a double Ctrl+C to exit from an empty focused prompt
  useBindings(() => ({
    target: inputTarget,
    enabled: inputTarget() !== undefined && !props.disabled && store.prompt.input === "",
    bindings: [
      ...
      commands: [
        {
          ...
          run() {
            ...
            vim.resetVim() // cssltdcode_change - recalled history starts in insert mode
  ...
  async function submitInner() {
    ...
    // cssltdcode_change start - in-memory cost alert command
    ...
    // cssltdcode_change start - gate TUI sends on declared agent requirements
    ...
    if (store.mode === "shell") {
      ...
      void sdk.client.session.shell({
        sessionID,
        agent: local.agent.current()?.name ?? "", // cssltdcode_change
        model: {
          providerID: selectedModel.providerID,
    ...
    toast.dismiss() // cssltdcode_change - dismiss persistent config warning on first submit
    history.append({
      ...store.prompt,
      mode: currentMode,
    })
  ...
  async function pasteInputText(text: string) {
    ...
    if (
      (lineCount >= 5 || pastedContent.length > 800) && // cssltdcode_change #7252 delay paste summary
  ...
  function clearPrompt() {
    ...
    setStore("extmarkToPartIndex", new Map())
    vim.resetVim() // cssltdcode_change - don't leak stale vim mode/selection into an emptied prompt
  }

  // cssltdcode_change start - cost-alert logic lives under cssltdcode/; only prompt mutation stays here
  ...
  const highlight = createMemo(() => {
    ...
    return local.agent.color(agent.name ?? "") // cssltdcode_change
  })
  ...
  const spinnerDef = createMemo(() => {
    const agent =
      status().type !== "idle"
        ...
        : local.agent.current()
    const color = agent ? local.agent.color(agent.name ?? "") : theme.border // cssltdcode_change
    return {
      frames: createFrames({
        color,
        style: "blocks",
  ...
  return (
    <>
      <box ref={(r: BoxRenderable) => (anchor = r)} visible={props.visible !== false} width="100%">
        ...
        >
          ...
          >
            <textarea
              ...
              /* cssltdcode_change */ onCursorChange={() => {
                setCursorVersion((value) => value + 1)
                if (store.mode === "normal") auto()?.onCursorChange()
              }}
              /* cssltdcode_change - KeyEvent type for vim key routing */ onKeyDown={(e: KeyEvent) => {
                if (props.disabled) {
                  e.preventDefault()
                  return
                }
                // cssltdcode_change start - route keys through the vim layer when enabled
                if (vim.vimOnKey(e)) {
                  e.preventDefault()
                  e.stopPropagation()
                  return
                ...
                <Show when={local.agent.current()} fallback={<box height={1} />}>
                  {(agent) => (
                    <>
                      ...
                      {/* cssltdcode_change start - vim mode indicator */}
                      <VimModeIndicator
        ...
        <box
          height={1}
          /* cssltdcode_change */ flexShrink={0}
          border={["left"]}
          borderColor={borderHighlight()}
          customBorderChars={{
            ...EmptyBorder,
        ...
        <box width="100%" flexDirection="row" justifyContent="space-between">
          ...
          <Show when={status().type !== "retry"}>
            <box gap={2} flexDirection="row">
              {/* cssltdcode_change start - show "ctrl+c again to exit" hint */}
