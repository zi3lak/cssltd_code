...
import { CSSLTD_TIPS } from "@/cssltdcode/cli/cmd/tui/feature-plugins/home/tips" // cssltdcode_change
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("cssltdcode.status"),
  ...
  const tip = createMemo(() => {
    ...
    const tips = [...CSSLTD_TIPS, ...TIPS, process.platform !== "win32" ? TERMINAL_SUSPEND_TIP : INPUT_UNDO_TIP].flatMap(
      (item) => {
...
const TIPS: Tip[] = [
  /* cssltdcode_change hide the entire list for if it is accidentally used
  "Type {highlight}@{/highlight} followed by a filename to fuzzy search and attach files",
  "Start a message with {highlight}!{/highlight} to run shell commands directly (e.g., {highlight}!ls -la{/highlight})",
  (shortcuts) => press(shortcuts.agentCycle(), "to cycle between Build and Plan agents"),
  "Use {highlight}/undo{/highlight} to revert the last message and file changes",
  "Use {highlight}/redo{/highlight} to restore previously undone messages and file changes",
  "Run {highlight}/share{/highlight} to create a public link to your conversation at cssltdcode.ai",
  ...
  "Create {highlight}cssltdcode.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/cssltdcode/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.cssltd/commands/{/highlight} to define reusable custom prompts", // cssltdcode_change
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.cssltd/agents/{/highlight} for specialized AI personas", // cssltdcode_change
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.cssltd/tools/{/highlight} to define new LLM tools", // cssltdcode_change
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.cssltd/plugins/{/highlight} for event hooks", // cssltdcode_change
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent CssltdCode from reading sensitive files",
  "Use {highlight}cssltdcode run{/highlight} for non-interactive scripting",
  "Use {highlight}cssltdcode --continue{/highlight} to resume the last session",
  "Use {highlight}cssltdcode run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}cssltdcode serve{/highlight} for headless API access to CssltdCode",
  "Use {highlight}cssltdcode run --attach{/highlight} to connect to a running server",
  "Run {highlight}cssltdcode upgrade{/highlight} to update to the latest version",
  "Run {highlight}cssltdcode auth list{/highlight} to see all configured providers",
  "Run {highlight}cssltdcode agent create{/highlight} for guided agent creation",
  "Use {highlight}/cssltdcode{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}cssltdcode github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/cssltdcode fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.cssltd/themes/{/highlight} directory", // cssltdcode_change
  ...
  "Run {highlight}cssltdcode debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/cssltdcode{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with CssltdCode Zen for curated, tested models",
