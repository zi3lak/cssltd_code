...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "cssltdcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "cssltdcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "cssltdcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "cssltdcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["cssltdcode.json", "cssltdcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "cssltdcode.json"),
        path.join(project, "cssltdcode.jsonc"),
      ])
    })
  })
