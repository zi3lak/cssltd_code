// Subprocess test harness for the cssltdcode CLI. Spawns the real binary against
...
//
// Configuration flows through cssltdcode's built-in test affordances:
//   - CSSLTD_CONFIG_CONTENT      : provider config inline, no files to find
//   - CSSLTD_TEST_HOME           : pins os.homedir() → tmpdir
//   - CSSLTD_DISABLE_PROJECT_CONFIG : skip walking up for cssltdcode.json
//   - CSSLTD_PURE                : skip external plugin discovery + install
//   - CSSLTD_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `cssltdcode.run` is fully wired. The shape supports adding more
// builders (`cssltdcode.serve(opts)`, `cssltdcode.acp(opts)`, `cssltdcode.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of CssltdcodeCli.
...
// Typed equivalent of constructing argv for `cssltdcode run`. New flags should
...
// `cssltdcode serve` is a long-lived process — it never exits on its own.
...
// `cssltdcode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type CssltdcodeCli = {
  // High-level: run a single prompt against the test model. Short-lived.
  readonly run: (message: string, opts?: RunOpts) => Effect.Effect<RunResult>
  // Spawn `cssltdcode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `cssltdcode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly cssltdcode: CssltdcodeCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("cssltdcode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("cssltdcode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "cssltdcode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `cssltdcode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("cssltdcode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const cssltdcode: CssltdcodeCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, cssltdcode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `cssltdcode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "cssltdcode") {
...
// every call site. Use this for any test that needs the cssltdcode CLI fixture.
//
...
// resources (e.g. `cssltdcode.serve`) without an extra `Effect.scoped` wrapper —
