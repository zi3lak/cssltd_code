...
describe("Session.Info", () => {
  ...
  test("round-trips every optional field", () => {
    const input = {
      ...
      workspaceID,
      directory: "/tmp/proj",
      path: "packages/cssltdcode",
      parentID: sessionIDChild,
      summary: {
        additions: 10,
        deletions: 5,
        files: 2,
        diffs: [{ additions: 1, deletions: 0, file: "a.ts" }], // cssltdcode_change
      },
  ...
  test("retry carries attempt/message/action/next", () => {
    const input = {
      ...
      action: {
        ...
        message: "Subscribe to CssltdCode Go.", // cssltdcode_change
        label: "subscribe",
        link: "https://cssltdcode.ai/go", // cssltdcode_change
      },
      next: 500,
    }
    expect(decode(input)).toEqual(input)
