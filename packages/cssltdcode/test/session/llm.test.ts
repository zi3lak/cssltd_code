...
import { USER_AGENT } from "../../src/installation" // cssltdcode_change
import { ProviderV2 } from "@cssltdcode/core/provider"
import { ModelV2 } from "@cssltdcode/core/model"

type ConfigModel = NonNullable<NonNullable<NonNullable<ConfigV1.Info["provider"]>[string]>["models"]>[string] // cssltdcode_change
...
describe("session.llm.ai-sdk adapter", () => {
  ...
  // cssltdcode_change start - preserve AI SDK raw usage for Cssltd provider billing
  ...
  test("preserves providerMetadata on step-finish so Anthropic cache writes survive getUsage", async () => {
    ...
    expect(stepFinish.providerMetadata).toEqual({
      ...
      cssltdcode: { routedModelID: "claude-3-5-sonnet" },
    })
    // cssltdcode_change end
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(headers.get("User-Agent") ?? "").toMatch(/^Cssltd-Code\//) // cssltdcode_change
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const capture = yield* Effect.promise(() => request)
        const body = capture.body
        const headers = capture.headers // cssltdcode_change

        expect(capture.url.pathname.endsWith("/messages")).toBe(true)
        expect(headers.get("User-Agent")?.split(" ")[0]).toBe(USER_AGENT) // cssltdcode_change
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // cssltdcode_change start - auth keys use the same Google API key header as Standard keys
