...
import { LLMEvent, ToolFailure } from "@cssltdcode/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@cssltdcode/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("cssltdcode"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("cssltdcode") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("cssltdcode") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, cssltdcode, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored cssltdcode auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("cssltdcode") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("cssltdcode"),
