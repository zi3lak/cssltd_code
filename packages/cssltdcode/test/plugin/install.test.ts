...
describe("plugin.install.task", () => {
  test("writes both server and tui config entries", async () => {
    ...
    const server = await read(path.join(tmp.path, ".cssltd", "cssltdcode.jsonc"))
    const tui = await read(path.join(tmp.path, ".cssltd", "tui.jsonc"))
  ...
  test("preserves JSONC comments when adding plugins to server and tui config", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server", "tui"])
    const cfg = path.join(tmp.path, ".cssltd") // cssltdcode_change
    const server = path.join(cfg, "cssltdcode.jsonc")
...
`,
  ...
  test("preserves JSONC comments when force replacing plugin version", async () => {
    ...
    const cfg = path.join(tmp.path, ".cssltd", "cssltdcode.jsonc") // cssltdcode_change
...
`,
  ...
  test("supports resolver target pointing to a file", async () => {
    ...
    const ok = await run(ctx(tmp.path))
    expect(ok).toBe(true)
    const server = await read(path.join(tmp.path, ".cssltd", "cssltdcode.jsonc")) // cssltdcode_change
    expect(server.plugin).toEqual(["acme@1.2.3"])
  })
  ...
  test("does not change configured package version without force", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server"])
    const cfg = path.join(tmp.path, ".cssltd", "cssltdcode.json") // cssltdcode_change
    await fs.mkdir(path.dirname(cfg), { recursive: true })
  ...
  test("writes to global scope when global flag is set", async () => {
    ...
    expect(await Filesystem.exists(path.join(global, "cssltdcode.jsonc"))).toBe(true)
    expect(await Filesystem.exists(path.join(tmp.path, ".cssltd", "cssltdcode.jsonc"))).toBe(false) // cssltdcode_change
  })
  ...
  test("writes local scope under directory when vcs is not git", async () => {
    ...
    expect(ok).toBe(true)
    // cssltdcode_change start
    expect(await Filesystem.exists(path.join(directory, ".cssltd", "cssltdcode.jsonc"))).toBe(true)
    expect(await Filesystem.exists(path.join(worktree, ".cssltd", "cssltdcode.jsonc"))).toBe(false)
    // cssltdcode_change end
  })
  ...
  test("writes local scope under directory when worktree is root slash", async () => {
    ...
    expect(ok).toBe(true)
    expect(await Filesystem.exists(path.join(directory, ".cssltd", "cssltdcode.jsonc"))).toBe(true) // cssltdcode_change
  })
  ...
  test("writes tui local scope under directory when worktree is root slash", async () => {
    ...
    const ok = await run(ctxRoot(directory))
    expect(ok).toBe(true)
    expect(await Filesystem.exists(path.join(directory, ".cssltd", "tui.jsonc"))).toBe(true) // cssltdcode_change
  })
  ...
  test("writes only tui config for tui-only plugins", async () => {
    ...
    expect(ok).toBe(true)
    // cssltdcode_change start
    expect(await Filesystem.exists(path.join(tmp.path, ".cssltd", "tui.jsonc"))).toBe(true)
    expect(await Filesystem.exists(path.join(tmp.path, ".cssltd", "cssltdcode.jsonc"))).toBe(false)
    // cssltdcode_change end
  })
  ...
  test("writes tui config for oc-themes-only packages", async () => {
    ...
    const tui = await read(path.join(tmp.path, ".cssltd", "tui.jsonc")) // cssltdcode_change
    expect(tui.plugin).toEqual(["acme@1.2.3"])
  })
  ...
  test("returns false for oc-themes outside plugin directory", async () => {
    ...
    // cssltdcode_change start
    expect(await Filesystem.exists(path.join(tmp.path, ".cssltd", "tui.jsonc"))).toBe(false)
    expect(await Filesystem.exists(path.join(tmp.path, ".cssltd", "cssltdcode.jsonc"))).toBe(false)
    // cssltdcode_change end
  })
  ...
  test("force replaces version in both server and tui configs", async () => {
    ...
    const target = await plugin(tmp.path, ["server", "tui"])
    // cssltdcode_change start
    const server = path.join(tmp.path, ".cssltd", "cssltdcode.json")
    const tui = path.join(tmp.path, ".cssltd", "tui.json")
    // cssltdcode_change end
