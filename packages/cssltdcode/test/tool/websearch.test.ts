...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.CSSLTD_WEBSEARCH_PROVIDER

    try {
      process.env.CSSLTD_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.CSSLTD_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.CSSLTD_WEBSEARCH_PROVIDER
      else process.env.CSSLTD_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is only enabled for cssltd or explicit websearch provider flags", () => {
    // cssltdcode_change
    expect(webSearchEnabled(ProviderV2.ID.cssltd, { exa: false, parallel: false })).toBe(true) // cssltdcode_change
    expect(webSearchEnabled(ProviderV2.ID.cssltdcode, { exa: false, parallel: false })).toBe(false) // cssltdcode_change
