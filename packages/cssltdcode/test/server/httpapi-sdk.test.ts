...
import { awaitWithTimeout, pollWithTimeout, testEffect } from "../lib/effect" // cssltdcode_change
...
type Sdk = ReturnType<typeof createCssltdClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.CSSLTD_SERVER_PASSWORD = input?.password
      Flag.CSSLTD_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".cssltd", "skills", "project-rest-skill", "SKILL.md"), // cssltdcode_change
      `---
...
describe("HttpApi SDK", () => {
  ...
  httpapiInstance(
    ...
    ({ sdk, directory }) =>
      Effect.gen(function* () {
        ...
        const raw = yield* call(() => sdk.v2.fs.read({ path: "hello.txt" })) // cssltdcode_change
        const session = yield* call(() => sdk.session.create({ title: "sdk" }))
        const v2session = yield* call(() => sdk.v2.session.create({ agent: "build" })) // cssltdcode_change
        ...
        expect({ status: v2session.response.status, error: v2session.error }).toEqual({ status: 200, error: undefined }) // cssltdcode_change
        expect(v2session.data).toMatchObject({ data: { location: { directory } } }) // cssltdcode_change
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        expect(request!.headers.has("x-cssltd-directory")).toBe(false)
        expect(request!.headers.has("x-cssltd-workspace")).toBe(false)

        // cssltdcode_change start - encoded legacy directory headers still route on payload requests
        const legacy = yield* client("raw", undefined, {
          headers: { "x-cssltd-directory": encodeURIComponent(directory) },
        })
  ...
  httpapiInstance(
    ...
    ({ directory }) =>
      Effect.gen(function* () {
        ...
        // cssltdcode_change start - match Hono AuthMiddleware username default ("cssltd")
        const badSdk = yield* client("raw", directory, {
          password: "secret",
          headers: { authorization: authorization("cssltd", "wrong") },
        })
        const bad = yield* capture(() => badSdk.file.read({ path: "hello.txt" }))
        const goodSdk = yield* client("raw", directory, {
          password: "secret",
          headers: { authorization: authorization("cssltd", "secret") },
        })
  ...
  // cssltdcode_change start - verify provider errors remain in successful assistant messages
