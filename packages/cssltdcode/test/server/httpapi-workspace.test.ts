    ...
    Effect.gen(function* () {
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          body: JSON.stringify({ $schema: "https://cssltdcode.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-cssltd-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-cssltd-workspace")
