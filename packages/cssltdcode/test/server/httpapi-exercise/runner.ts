    ...
    Effect.flatMap((context) =>
      Effect.gen(function* () {
        ...
        const base: ScenarioContext = {
          ...
          headers: (extra) => ({
            ...(context.dir?.path ? { "x-cssltd-directory": context.dir.path } : {}),
            ...extra,
          }),
          file: (name, content) =>
            Effect.promise(() => {
...
const resetState = Effect.promise(async () => {
  ...
  // cssltdcode_change - each exerciser process already owns an isolated DB; unlinking it between scenarios races async Cssltd callbacks
  await Bun.sleep(25)
})
