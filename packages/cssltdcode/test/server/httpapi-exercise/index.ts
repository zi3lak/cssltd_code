 ...
 *
 * The script intentionally isolates `CSSLTD_DB` before importing modules that touch
...
import { cssltdScenarios } from "../../cssltdcode/server/httpapi-exercise-scenarios" // cssltdcode_change
...
const scenarios: Scenario[] = [
  ...
  http.protected
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "cssltdcode.jsonc"),
      ...
      (body) =>
        Effect.gen(function* () {
          ...
          const text = yield* Effect.promise(() =>
            Bun.file(path.join(exerciseConfigDirectory, "cssltdcode.jsonc")).text(),
          )
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-cssltd-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-cssltd-directory")
  }),
  ...
  http.protected.get("/cssltdcode/notebook", "cssltdcode.notebook.list").json(200, array),
  http.protected
    .post("/cssltdcode/notebook/{requestID}/reply", "cssltdcode.notebook.reply")
    .at((ctx) => ({
      path: route("/cssltdcode/notebook/{requestID}/reply", { requestID: "nbr_httpapi_reply" }),
      headers: ctx.headers(),
      body: {
        result: {
          operation: "read",
    ...
    .json(404, object, "status"),
  http.protected
    .post("/cssltdcode/notebook/{requestID}/reject", "cssltdcode.notebook.reject")
    .at((ctx) => ({
      path: route("/cssltdcode/notebook/{requestID}/reject", { requestID: "nbr_httpapi_reject" }),
      headers: ctx.headers(),
      body: { error: { code: "not_found", message: "Notebook not found" } },
    }))
    .json(404, object, "status"),
  http.protected.get("/cssltdcode/agent-manager", "cssltdcode.agentManager.list").json(200, array),
  http.protected
    .post("/cssltdcode/agent-manager/{requestID}/reply", "cssltdcode.agentManager.reply")
    .at((ctx) => ({
      path: route("/cssltdcode/agent-manager/{requestID}/reply", { requestID: "amr_httpapi_reply" }),
      headers: ctx.headers(),
      body: { result: { operation: "overview", overview: { sections: [], ungrouped: [] } } },
    }))
    .json(404, object, "status"),
  http.protected
    .post("/cssltdcode/agent-manager/{requestID}/reject", "cssltdcode.agentManager.reject")
    .at((ctx) => ({
      path: route("/cssltdcode/agent-manager/{requestID}/reject", { requestID: "amr_httpapi_reject" }),
      headers: ctx.headers(),
  ...
  http.protected
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "cssltdcode", model: "test" })}`,
      headers: ctx.headers(),
    }))
    .json(200, array, "status"),
  ...
  http.protected
    ...
    .status(500, undefined, "status"), // cssltdcode_change
  http.protected
  ...
  http.protected
    ...
    .status(400),
  ...cssltdScenarios, // cssltdcode_change
]
