...
const agentLayer = (flags: Partial<RuntimeFlags.Info> = {}) =>
  Agent.layer.pipe(
    ...
    Layer.provide(LocationServiceMap.layer), // cssltdcode_change
    Layer.provide(RuntimeFlags.layer(flags)),
  )

const it = testEffect(agentLayer())
const scout = testEffect(agentLayer({ experimentalScout: true })) // cssltdcode_change
...
it.instance("returns default native agents when no config", () =>
  Effect.gen(function* () {
    ...
    expect(names).not.toContain("scout") // cssltdcode_change
...
it.instance("build agent has correct default properties", () =>
  Effect.gen(function* () {
    ...
    expect(evalPerm(build, "interactive_terminal")).toBe("allow") // cssltdcode_change
  }),
)

it.instance("plan agent denies edits except .cssltdcode/plans/*", () =>
  Effect.gen(function* () {
    const plan = yield* load((svc) => svc.get("plan"))
    expect(plan).toBeDefined()
    // Wildcard is denied
    expect(evalPerm(plan, "edit")).toBe("deny")
    expect(evalPerm(plan, "interactive_terminal")).toBe("deny") // cssltdcode_change
    // But specific path is allowed
    expect(Permission.evaluate("edit", ".cssltdcode/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance("explore agent denies edit and write", () =>
  Effect.gen(function* () {
    ...
    expect(evalPerm(explore, "interactive_terminal")).toBe("deny") // cssltdcode_change
  }),
)
...
// cssltdcode_change start - Scout is opt-in and owns repository research permissions
...
scout.instance(
  ...
  {
    config: {
      // cssltdcode_change - Scout-backed Cssltd agents use the supported references config
      references: {
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".cssltd", "skill", "perm-skill") // cssltdcode_change
...
`,
      ...
      const home = process.env.CSSLTDCODE_TEST_HOME
      process.env.CSSLTDCODE_TEST_HOME = test.directory
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => {
          process.env.CSSLTDCODE_TEST_HOME = home
        }),
      )
