...
describe("ProviderTransform.schema - gemini non-object properties removal", () => {
  ...
  test("sanitizes gemini schemas routed through the Cssltd Gateway", () => {
    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".cssltdcode/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const cssltdcodeModel = {
      ...openaiModel,
      providerID: "cssltdcode",
      api: {
        id: "cssltdcode-test",
        url: "https://api.cssltdcode.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, cssltdcodeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.cssltdcode?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.cssltdcode?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          cssltdcode: { itemId: "msg_cssltdcode" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              cssltdcode: { itemId: "msg_cssltdcode_part" },
    ...
    expect(result[0].providerOptions?.cssltdcode?.itemId).toBe("msg_cssltdcode")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.cssltdcode?.itemId).toBe("msg_cssltdcode_part")
...
describe("ProviderTransform.variants", () => {
  ...
  // cssltdcode_change start: minimax
  ...
  test("minimax via cssltd gateway returns instant/thinking toggle", () => {
    const model = createMockModel({
      id: "cssltd/minimax/minimax-m3",
      providerID: "cssltd",
      api: {
        id: "minimax/minimax-m3",
        url: "https://gateway.cssltd.ai",
  ...
  // cssltdcode_change start
  describe("@cssltdcode/cssltd-gateway", () => {
    test("claude models return empty variants (reasoning disabled)", () => {
      const model = createMockModel({
        id: "cssltd/anthropic/claude-sonnet-4",
        providerID: "cssltd",
        capabilities: { reasoning: false },
        api: {
          id: "anthropic/claude-sonnet-4",
    ...
    test("anthropic models in api.id return empty variants (reasoning disabled)", () => {
      const model = createMockModel({
        id: "cssltd/anthropic/claude-opus-4",
        providerID: "cssltd",
        capabilities: { reasoning: false },
        api: {
    ...
    test("codex models return OPENAI_EFFORTS with object-based reasoning format", () => {
      const model = createMockModel({
        id: "cssltd/openai/gpt-5.2-codex",
        providerID: "cssltd",
        api: {
    ...
    // cssltdcode_change start
    test("mercury-2 uses server-provided variants from cssltd gateway", () => {
  ...
  describe("@ai-sdk/azure", () => {
    ...
    for (const testCase of [
      { id: "o3-deep-research", efforts: ["medium"] }, // cssltdcode_change - preserve helper exclusions on Azure
      { id: "gpt-5-pro", efforts: ["high"] }, // cssltdcode_change - preserve helper exclusions on Azure
  ...
  describe("ProviderTransform.smallOptions", () => {
    describe("@cssltdcode/cssltd-gateway", () => {
      ...
      test("google models use their default reasoning effort", () => {
        const model = createMockModel({
          id: "cssltd/google/gemini-2.0-flash",
          providerID: "cssltd",
          api: {
...
// cssltdcode_change start - tests for reasoningSummary guard
describe("ProviderTransform.options - OpenAI Responses API params guard", () => {
  ...
  test("includes reasoningSummary for @cssltdcode/cssltd-gateway", () => {
    const result = ProviderTransform.options({
      model: gpt5Model("@cssltdcode/cssltd-gateway", "cssltd"),
      sessionID,
    })
    expect(result.reasoningSummary).toBe("auto")
  })
