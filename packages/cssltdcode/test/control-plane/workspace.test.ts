...
const originalEnv = {
  ...
  CSSLTD_EXPERIMENTAL_WORKSPACES: process.env.CSSLTD_EXPERIMENTAL_WORKSPACES,
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.CSSLTD_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=cssltdcode-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.CSSLTD_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.CSSLTD_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.CSSLTD_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=cssltdcode-test")
