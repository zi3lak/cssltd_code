...
import { capture } from "../../src/cssltdcode/instance" // cssltdcode_change
...
describe("InstanceStore", () => {
  it.live("loads instance context", () =>
    Effect.gen(function* () {
      ...
      // cssltdcode_change start - capture prefers legacy ALS, then falls back to the Effect fiber reference
  ...
  it.live("runs bootstrap with InstanceRef provided", () =>
    Effect.gen(function* () {
      ...
      expect(capture()).toBeUndefined() // cssltdcode_change - bootstrap legacy ALS does not leak into the caller
    }),
  )
  ...
  // cssltdcode_change start - reload disposers retain legacy instance context
  ...
  // cssltdcode_change - InstanceStore.boot provides InstanceRef to bootstrap.run so
  // CssltdcodeBootstrap (and anything it forkDetaches, e.g. CssltdIndexing.init) can read
  // the current directory. This regression test pins the Cssltd contract.
  it.live("provides InstanceRef during bootstrap for Cssltd bootstrap compatibility", () =>
    Effect.gen(function* () {
      const dir = yield* tmpdirScoped({ git: true })
      const store = yield* InstanceStore.Service
