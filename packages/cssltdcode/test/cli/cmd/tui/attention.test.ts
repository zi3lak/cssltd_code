...
import { createTuiAttention } from "@cssltdcode/tui/attention"
...
describe("createTuiAttention", () => {
  ...
  test("supports focused-only requests", async () => {
    ...
    expect(renderer.notifications).toEqual([{ title: "Cssltd", message: "focused" }]) // cssltdcode_change
  })
  ...
  test("notification can deliver while focused when requested", async () => {
    ...
    expect(audio.playCalls).toBe(1)
    expect(renderer.notifications).toEqual([{ title: "Cssltd", message: "hello" }]) // cssltdcode_change
  })
  ...
  test("notifies while blurred", async () => {
    ...
    expect(await attention.notify({ title: "Cssltd", message: "hello", sound: false })).toEqual({
      // cssltdcode_change
      ok: true,
      notification: true,
      sound: false,
  ...
  test("can play sound always while notification is blurred-only", async () => {
    ...
    expect(renderer.notifications).toEqual([{ title: "Cssltd", message: "hello again" }]) // cssltdcode_change
  })
  ...
  test("plays named sounds from the active sound pack", async () => {
    ...
    dispose()
    expect(attention.soundboard.current()).toBe("cssltd.default") // cssltdcode_change
  })
