...
import type { TuiAttentionSoundPack } from "@cssltdcode/plugin/tui"
...
const { allThemes, addTheme } = await import("@cssltdcode/tui/context/theme")
...
async function load(): Promise<Data> {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const localDest = path.join(dir, ".cssltd", "themes", localThemeFile) // cssltdcode_change
      const globalDest = path.join(Global.Path.config, "themes", globalThemeFile)
      const preloadedDest = path.join(dir, ".cssltd", "themes", preloadedThemeFile) // cssltdcode_change
...
`,
  ...
  try {
    ...
    const leaked_global_to_local = await fs
      .stat(path.join(tmp.path, ".cssltd", "themes", tmp.extra.globalThemeFile)) // cssltdcode_change
      .then(() => true)
      .catch(() => false)
...
`,
  ...
  process.env.CSSLTD_PLUGIN_META_FILE = path.join(tmp.path, "plugin-meta.json")
  const config = createTuiResolvedConfig({
    plugin: [
...
test("does not bootstrap server plugins while initializing tui plugins", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(path.join(dir, "cssltdcode.json"), JSON.stringify({ plugin: [pathToFileURL(plugin).href] }))
      return { marker }
    },
  })
...
test("updates installed theme when plugin metadata changes", async () => {
  await using tmp = await tmpdir<{
    ...
    init: async (dir) => {
      ...
      const dest = path.join(dir, ".cssltd", "themes", themeFile) // cssltdcode_change
...
`,
    ...
    const list = await Filesystem.readJson<Record<string, { themes?: Record<string, { dest: string }> }>>(
      process.env.CSSLTD_PLUGIN_META_FILE!,
    )
