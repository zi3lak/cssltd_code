...
`,
  ...
  const pure = process.env.CSSLTD_PURE
  const meta = process.env.CSSLTD_PLUGIN_META_FILE
  process.env.CSSLTD_PURE = "1"
  process.env.CSSLTD_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.CSSLTD_PURE
    } else {
      process.env.CSSLTD_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.CSSLTD_PLUGIN_META_FILE
    } else {
      process.env.CSSLTD_PLUGIN_META_FILE = meta
    }
  }
})
