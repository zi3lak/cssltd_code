...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/cssltdcode.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/cssltdcode.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/cssltdcode.jsonc", dir: ".cssltdcode", suggestion: "cssltdcode" },
        expected:
          'Directory ".cssltdcode" in /tmp/cssltdcode.jsonc is not valid. Rename the directory to "cssltdcode" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/cssltdcode.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/cssltdcode.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats account transport errors clearly", () => {
    const error = new AccountTransportError({
      method: "POST",
      url: "https://console.cssltdcode.ai/auth/device/code",
    })

    const formatted = FormatError(error)

    expect(formatted).toContain("Could not reach POST https://console.cssltdcode.ai/auth/device/code.")
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `cssltd models` to list available models",
      "Or check your cssltd.json provider/model names", // cssltdcode_change
    ].join("\n")
