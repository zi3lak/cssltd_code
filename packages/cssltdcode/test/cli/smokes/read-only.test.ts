...
describe("cssltdcode read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // CSSLTD_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ cssltdcode }) =>
      Effect.gen(function* () {
        const r = yield* cssltdcode.spawn(["mcp", "list"])
        cssltdcode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ cssltdcode }) =>
      Effect.gen(function* () {
        const r = yield* cssltdcode.spawn(["providers", "list"])
        cssltdcode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ cssltdcode }) =>
      Effect.gen(function* () {
        const r = yield* cssltdcode.spawn(["models"])
        cssltdcode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ cssltdcode }) =>
      Effect.gen(function* () {
        const r = yield* cssltdcode.spawn(["agent", "list"])
        cssltdcode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh CSSLTD_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ cssltdcode }) =>
      Effect.gen(function* () {
        const r = yield* cssltdcode.spawn(["session", "list"])
        cssltdcode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ cssltdcode }) =>
      Effect.gen(function* () {
        const r = yield* cssltdcode.spawn(["stats"])
        cssltdcode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under CSSLTD_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ cssltdcode }) =>
      Effect.gen(function* () {
        const r = yield* cssltdcode.spawn(["db", "path"])
        cssltdcode.expectExit(r, 0, "db path")
