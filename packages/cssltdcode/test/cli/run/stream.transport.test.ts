...
import { CssltdClient, type GlobalEvent } from "@cssltdcode/sdk/v2"
...
type GlobalEventStream = Awaited<ReturnType<CssltdClient["global"]["event"]>>["stream"]
type SessionMessage = NonNullable<Awaited<ReturnType<CssltdClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<CssltdClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<CssltdClient["session"]["status"]>>["data"]>
...
function sdk(
  input: {
    ...
    globalEvent?: CssltdClient["global"]["event"]
    promptAsync?: CssltdClient["session"]["promptAsync"]
    status?: CssltdClient["session"]["status"]
    messages?: CssltdClient["session"]["messages"]
    children?: CssltdClient["session"]["children"]
    permissions?: CssltdClient["permission"]["list"]
    questions?: CssltdClient["question"]["list"]
  } = {},
) {
  const client = new CssltdClient()

  const globalEvent: CssltdClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: CssltdClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: CssltdClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: CssltdClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: CssltdClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: CssltdClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: CssltdClient["question"]["list"] = input.questions ?? (() => ok([]))
