...
import type { HostPluginApi } from "@cssltdcode/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  let selected = opts.theme?.selected ?? "cssltdcode"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "cssltd.default", // cssltdcode_change
    ...
    state: {
      ...
      session: {
        ...
        processes: opts.state?.session?.processes ?? (() => []), // cssltdcode_change
