...
import * as PowerShell from "@/cssltdcode/shell/shell" // cssltdcode_change - PowerShell args
...
export function gitbash() {
  ...
  if (Flag.CSSLTD_GIT_BASH_PATH) return Flag.CSSLTD_GIT_BASH_PATH
  const git = which("git")
  if (!git) return
...
export function args(file: string, command: string, cwd: string) {
  ...
  if (ps(file)) return PowerShell.args(command) // cssltdcode_change - PowerShell args
  return ["-c", command]
}
