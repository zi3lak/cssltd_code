...
import * as CssltdOAuthCallback from "../cssltdcode/mcp-oauth-callback" // cssltdcode_change

const log = Log.create({ service: "mcp.oauth-callback" }) // cssltdcode_change
...
<head>
  ...
  <title>Cssltd - Authorization Successful</title>
...
<head>
  <!-- cssltdcode_change start -->
  <title>Cssltd - Authorization Failed</title>
  <!-- cssltdcode_change end -->
  <style>
    body { font-family: system-ui, -apple-system, sans-serif; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; background: #1a1a2e; color: #eee; }
    .container { text-align: center; padding: 2rem; }
...
export async function ensureRunning(redirectUri?: string): Promise<void> {
  // cssltdcode_change start - delegate Cssltd-specific callback binding from here because OAuth state lives in this module
  await CssltdOAuthCallback.ensureRunning({
    redirectUri,
  ...
  await new Promise<void>((resolve, reject) => {
    // cssltdcode_change start - EADDRINUSE can still fire when another process
    ...
    server!.listen(currentPort, () => {
      server!.off("error", onError) // cssltdcode_change
