...
import { EventSequenceTable, EventTable } from "@cssltdcode/core/event/sql" // cssltdcode_change - upstream moved the event tables to core
...
> = {
  ...
  wire?: boolean // cssltdcode_change - EventV2 rows cross persistence and bus boundaries as encoded data
}
...
// cssltdcode_change start - serialized rows carry the schema's encoded representation
...
export class Service extends Context.Service<Service, Interface>()("@cssltdcode/SyncEvent") {}
...
export const layer = Layer.effect(Service)(
  Effect.gen(function* () {
    ...
    const replay: Interface["replay"] = Effect.fn("SyncEvent.replay")(function* (event, options) {
      ...
      // cssltdcode_change start - decode only EventV2 rows
...
export function init(input: { projectors: Array<[Definition, ProjectorFunc]>; convertEvent?: ConvertEvent }) {
  ...
  for (let entry of EventV2.registry.values()) {
    if (!entry.sync) continue // cssltdcode_change - EventV2 stores legacy sync metadata under sync
    register({
      type: entry.type,
      version: entry.sync.version, // cssltdcode_change
      aggregate: entry.sync.aggregate, // cssltdcode_change
      properties: entry.data,
      schema: entry.data,
      wire: true, // cssltdcode_change
    })
  }
...
function process<Def extends Definition>(
  ...
  Database.transaction((tx) => {
    ...
    const data = def.wire ? EventWire.encode(def.schema, event.data) : event.data // cssltdcode_change
    ...
    if (options.experimentalWorkspaces) {
      ...
      tx.insert(EventTable)
        .values({
          id: EventV2.ID.make(event.id), // cssltdcode_change - core event table uses the branded EventV2 ID
          seq: event.seq,
          aggregate_id: event.aggregateID,
          type: versionedType(def.type, def.version),
          data: data as Record<string, unknown>, // cssltdcode_change
        })
        .run()
    }
    ...
    Database.effect(() => {
      ...
      const publish = (value: unknown) =>
        ...
        options.bridge.fork(
          Effect.gen(function* () {
            // cssltdcode_change start - encode EventV2 properties before crossing the legacy boundary
            if (def.wire) {
...
export function effectPayloads() {
  return [
    ...
    ...EventV2.registry
      ...
      .filter(
        (definition) =>
          definition.sync !== undefined && // cssltdcode_change
          !registry.has(versionedType(definition.type, definition.sync.version)), // cssltdcode_change
      )
      .map((definition) =>
        EffectSchema.Struct({
          type: EffectSchema.Literal("sync"),
          name: EffectSchema.Literal(versionedType(definition.type, definition.sync!.version)), // cssltdcode_change
          id: EffectSchema.String,
          seq: EffectSchema.Finite,
          aggregateID: EffectSchema.Literal(definition.sync!.aggregate), // cssltdcode_change
