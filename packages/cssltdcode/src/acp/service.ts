...
import type { Message, CssltdClient, SessionMessageResponse } from "@cssltdcode/sdk/v2"
...
export const AuthMethodID = "cssltd-login" // cssltdcode_change
...
export class Service extends Context.Service<Service, Interface>()("@cssltdcode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `cssltd auth login` in the terminal",
      name: "Login with Cssltd",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: CssltdClient) {
...
function makeUsageService(sdk: CssltdClient) {
...
async function loadDirectorySnapshot(sdk: CssltdClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, cssltdcode provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const cssltdProvider = providers[ProviderV2.ID.make("cssltd")] // cssltdcode_change
  const cssltdModel = cssltdProvider ? Provider.sort(Object.values(cssltdProvider.models))[0] : undefined // cssltdcode_change
  if (cssltdProvider && cssltdModel) return { providerID: cssltdProvider.id, modelID: cssltdModel.id } // cssltdcode_change
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "CssltdCode service failure", service })
}
