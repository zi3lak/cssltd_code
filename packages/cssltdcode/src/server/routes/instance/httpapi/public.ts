...
import { matchLegacyCssltdOpenApi } from "@/cssltdcode/server/httpapi/public"
import * as CssltdServer from "@/cssltdcode/server/server"
// cssltdcode_change end
import { CssltdCodeHttpApi } from "./api"
...
const QueryParameterSchemas: Record<string, OpenApiSchema> = {
  ...
  "GET /cssltd/cloud-sessions cursor": { type: "string" },
  "GET /cssltd/cloud-sessions limit": { type: "number" },
  // cssltdcode_change end
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for the cssltd serve command", // cssltdcode_change
...
function matchLegacyOpenApi(input: Record<string, unknown>) {
  ...
  matchLegacyCssltdOpenApi(input) // cssltdcode_change
  return input
}
...
function applyLegacySchemaOverrides(spec: OpenApiSpec) {
  ...
  // cssltdcode_change start - preserve model reset sentinels in indexing config patches
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(CssltdCodeHttpApi)
...
function normalizeParameter(param: OpenApiParameter, route: string) {
  ...
  if (param.in === "path") {
    param.schema = pathParameterSchema(route, param.name) ?? stripOptionalNull(param.schema) // cssltdcode_change
    return
  }
  if (param.in === "query") {
...
export const PublicApi = CssltdCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: CssltdServer.DOC_TITLE, // cssltdcode_change
    version: "1.0.0",
    description: CssltdServer.DOC_DESCRIPTION, // cssltdcode_change
