import { Image } from "@/image/image" // cssltdcode_change - classify user image validation defects
import { CssltdSessionHttpApi } from "@/cssltdcode/server/httpapi/session-fork" // cssltdcode_change
import { BlockedError as AgentRequirementError } from "@/cssltdcode/agent-requirements" // cssltdcode_change
...
import {
  ...
  SummarizePayload,
  UpdatePayload,
  ViewedPayload, // cssltdcode_change
} from "../groups/session"
...
export const sessionHandlers = HttpApiBuilder.group(InstanceHttpApi, "session", (handlers) =>
  Effect.gen(function* () {
    ...
    const viewers = yield* CssltdViewers.Service // cssltdcode_change
    ...
    const forkRaw = CssltdSessionHttpApi.forkRaw(fork) // cssltdcode_change - carry upstream bodyless full-session fork support
    ...
    const prompt = Effect.fn("SessionHttpApi.prompt")(function* (ctx: {
      ...
      const message = yield* promptSvc
        .prompt({ ...ctx.payload, sessionID: ctx.params.sessionID } as unknown as SessionPrompt.PromptInput) // cssltdcode_change
        .pipe(
          // cssltdcode_change start - reject only typed user image validation defects as request errors
    ...
    const promptAsync = Effect.fn("SessionHttpApi.promptAsync")(function* (ctx: {
      ...
      yield* promptSvc
        ...
        .pipe(
          Effect.catchCause((cause) => {
            if (Cause.hasInterruptsOnly(cause)) return Effect.void // cssltdcode_change - Stop is not an error
    ...
    return handlers
      ...
      .handleRaw("fork", forkRaw) // cssltdcode_change - carry upstream bodyless full-session fork support
      .handle("abort", abort)
      .handle("init", init)
      .handle("share", share)
      ...
      .handle("viewed", viewed) // cssltdcode_change
  }),
)
