...
import { serverUrls } from "@/cssltdcode/cli/server-urls" // cssltdcode_change
...
import * as CssltdListener from "@/cssltdcode/server/listener" // cssltdcode_change
...
export async function listen(opts: ListenOptions): Promise<Listener> {
  ...
  return {
    ...
    port: listener.port,
    url: listener.url,
    urls: listener.urls, // cssltdcode_change
...
const listenEffect: (opts: ListenOptions) => Effect.Effect<EffectListener, unknown> = Effect.fn("Server.listen")(
  function* (opts: ListenOptions) {
    ...
    return {
      ...
      port: address.port,
      url: listenerUrl,
      urls: serverUrls(opts.hostname, address.port), // cssltdcode_change
...
function listenerLayer(opts: ListenOptions, port: number) {
  return HttpRouter.serve(HttpApiApp.createListenerRoutes(opts), { // cssltdcode_change
...
function startListener(opts: ListenOptions, port: number) {
  ...
  return CssltdListener.build(listenerLayer(opts, port), scope).pipe( // cssltdcode_change
