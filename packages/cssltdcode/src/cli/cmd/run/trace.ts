...
//
// Enable with CSSLTD_DIRECT_TRACE=1. Writes one JSON line per event to
// ~/.local/share/cssltd/log/direct/<timestamp>-<pid>.jsonl. Also writes // cssltdcode_change
...
export function trace(): Trace | undefined {
  ...
  if (!process.env.CSSLTD_DIRECT_TRACE) {
    state = false
    return undefined
  }
