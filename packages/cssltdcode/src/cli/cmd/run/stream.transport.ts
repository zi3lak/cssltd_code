...
import type { GlobalEvent, CssltdClient } from "@cssltdcode/sdk/v2"
...
class Service extends Context.Service<Service, TransportService>()("@cssltdcode/RunStreamTransport") {}
...
function sid(event: Event): string | undefined {
  ...
  if (event.type === "interactive_terminal.updated") return event.properties.info.sessionID // cssltdcode_change
  ...
  if (
    ...
    event.type === "interactive_terminal.data" || // cssltdcode_change
    event.type === "interactive_terminal.deleted" || // cssltdcode_change
...
function pickView(data: SessionData, subagent: SubagentData, order: Map<string, number>): FooterView {
  return pickBlockerView({
    terminal: data.terminal, // cssltdcode_change
