...
import { GitHubSecurity } from "@/cssltdcode/security/github" // cssltdcode_change
...
const AGENT_USERNAME = "cssltdconnect[bot]" // cssltdcode_change
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/cssltd.yml" // cssltdcode_change
  ...
  yield* Effect.promise(async () => {
    {
      ...
      function printNextSteps() {
        ...
        prompts.outro(
          [
            ...
            step2,
            "",
            "    3. Go to a GitHub issue and comment `/cssltd summarize` to see the agent in action", // cssltdcode_change
            "",
            "   Learn more about the GitHub agent - https://cssltd.ai/docs/code-with-ai/platforms/github", // cssltdcode_change
          ].join("\n"),
        )
      }
      ...
      async function installGitHubApp() {
        ...
        // Open browser
        const url = "https://github.com/apps/cssltdconnect" // cssltdcode_change
        const command =
        ...
        async function getInstallation() {
          // cssltdcode_change start - updated to new endpoint
          return await fetch(`https://api.cssltd.ai/api/integrations/github/check-installation?owner=${app.owner}`)
            .then((res) => res.json())
      ...
      async function addWorkflowFiles() {
        // cssltdcode_change start - updated workflow template with Cssltd branding and gateway secrets
        const providerEnvStr =
          provider === "amazon-bedrock"
            ? ""
            : providers[provider].env.map((e) => `\n          ${e}: \${{ secrets.${e} }}`).join("")

        const cssltdGatewayEnv =
          provider === "cssltd"
            ? `\n          CSSLTD_API_KEY: \${{ secrets.CSSLTD_API_KEY }}\n          CSSLTD_ORG_ID: \${{ secrets.CSSLTD_ORG_ID }}`
            : ""

        const envStr = providerEnvStr || cssltdGatewayEnv ? `\n        env:${providerEnvStr}${cssltdGatewayEnv}` : ""
...
jobs:
  cssltd:
    if: |
      ...
      contains(github.event.comment.body, ' /cssltd') ||
      startsWith(github.event.comment.body, '/cssltd')
    ...
    steps:
      ...
      - name: Run Cssltd
        uses: Cssltd-Org/cssltdcode/github@latest${envStr}
        with:
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    const shareBaseUrl = isMock ? "https://dev.cssltd.ai" : "https://cssltd.ai" // cssltdcode_change
    ...
    try {
      ...
      // Setup cssltd session // cssltdcode_change
      ...
      console.log("cssltd session", session.id) // cssltdcode_change
    ...
    function normalizeOidcBaseUrl(): string {
      ...
      if (!value) return "https://api.cssltd.ai" // cssltdcode_change
    ...
    async function getUserPrompt() {
      ...
      const mentions = (process.env["MENTIONS"] || "/cssltd,/kc") // cssltdcode_change
        .split(",")
      ...
      for (const m of matches) {
        const tag = m[0]
        // cssltdcode_change start - only fetch canonical GitHub attachment routes
    ...
    async function chat(message: string, files: PromptFiles = []) {
      console.log("Sending message to cssltd...") // cssltdcode_change
    ...
    async function getOidcToken() {
      try {
        return await core.getIDToken("cssltd-github-action") // cssltdcode_change
      } catch (error) {
    ...
    async function exchangeForAppToken(token: string) {
      // cssltdcode_change start - updated endpoint URLs per new API structure
    ...
    function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
      ...
      if (type === "schedule" || type === "dispatch") {
        ...
        return `cssltd/${type}-${hex}-${timestamp}` // cssltdcode_change
      }
      return `cssltd/${type}${issueId}-${timestamp}` // cssltdcode_change
    }
    ...
    function footer(opts?: { image?: boolean }) {
      // cssltdcode_change start - simplified footer with text branding (no image backend yet)
      const share = shareId ? `[cssltd session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
      return `\n\n---\n*Powered by [Cssltd](https://cssltd.ai)*&nbsp;&nbsp;|&nbsp;&nbsp;${share}[github run](${runUrl})`
      // cssltdcode_change end
    }
  ...
  repository(owner: $owner, name: $repo) {
    ...
    function buildPromptDataForIssue(issue: GitHubIssue) {
      ...
      return [
        ...
        "- Git push and PR creation are handled AUTOMATICALLY by the cssltd infrastructure after your response", // cssltdcode_change
