...
import { CssltdcodeMcpConfig } from "@/cssltdcode/cli/cmd/mcp" // cssltdcode_change
...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: cssltd mcp add") // cssltdcode_change
      return
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in cssltd.json:") // cssltdcode_change
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // cssltdcode_change start - prefer supported Cssltd config directories over root files
  const roots = [
    path.join(baseDir, "cssltd.jsonc"),
    path.join(baseDir, "cssltd.json"),
    path.join(baseDir, "cssltdcode.jsonc"),
    path.join(baseDir, "cssltdcode.json"),
  ]
  const candidates = global
    ? roots
    : [
        path.join(baseDir, ".cssltd", "cssltd.jsonc"),
        path.join(baseDir, ".cssltd", "cssltd.json"),
        path.join(baseDir, ".cssltd", "cssltdcode.jsonc"),
        path.join(baseDir, ".cssltd", "cssltdcode.json"),
        path.join(baseDir, ".cssltdcode", "cssltd.jsonc"),
        path.join(baseDir, ".cssltdcode", "cssltd.json"),
        path.join(baseDir, ".cssltdcode", "cssltdcode.jsonc"),
        path.join(baseDir, ".cssltdcode", "cssltdcode.json"),
        ...roots,
      ]
  ...
  // Default to cssltd.json if none exist
  return path.join(baseDir, "cssltd.json")
...
async function addMcpToConfig(name: string, mcpConfig: ConfigMCPV1.Info, configPath: string) {
  ...
  const result = CssltdcodeMcpConfig.format(configPath, applyEdits(text, edits)) // cssltdcode_change
...
export const McpAddCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.add")(function* (args) {
    ...
    const ctx = maybeCtx
    const global = Flag.CSSLTD_CONFIG_DIR ?? Global.Path.config // cssltdcode_change - honor the active Cssltd config profile
    yield* Effect.promise(async () => {
      ...
      if (args.name) {
        ...
        const configPath = await resolveConfigPath(global, true) // cssltdcode_change
      ...
      const [projectConfigPath, globalConfigPath] = await Promise.all([
        ...
        resolveConfigPath(global, true), // cssltdcode_change
      ])
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "e.g., cssltd x @modelcontextprotocol/server-filesystem", // cssltdcode_change
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "cssltd-debug", version: InstallationVersion }, // cssltdcode_change
            },
            id: 1,
          }),
        })
        ...
        if (response.status === 401) {
          ...
          try {
            const client = new Client({
              name: "cssltd-debug", // cssltdcode_change
