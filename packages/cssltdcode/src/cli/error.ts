...
export function FormatError(input: unknown): string | undefined {
  ...
  if (NamedError.hasName(input, "MCPFailed")) {
    ...
    return `MCP server "${data}" failed.` // cssltdcode_change
  }
  ...
  // ProviderModelNotFoundError: { providerID: string, modelID: string, suggestions?: string[], modelsEmpty?: boolean } // cssltdcode_change
  ...
  if (providerModelNotFound) {
    ...
    return [
      ...
      ...(providerModelNotFound.modelsEmpty === true ? ["No models are currently available."] : []), // cssltdcode_change
  ...
  if (remoteAuth) {
    ...
    return [
      ...
      ...(url ? [`Run \`cssltd auth login ${url}\` to re-authenticate.`] : []), // cssltdcode_change
    ].join("\n")
  }
