...
import { CssltdRunner } from "@/cssltdcode/effect/runner" // cssltdcode_change
...
export const make = <A, E = never>(
  ...
  // cssltdcode_change start - do not let work publish busy before the Running state is committed
  const startRun = (work: Effect.Effect<A, E>, done: Deferred.Deferred<A, E | Cancelled>) => {
    const id = next()
    return CssltdRunner.start({
      work,
      scope,
  ...
  // cssltdcode_change start - open work only after the Running state is committed
    ...
    SynchronizedRef.modifyEffect(
      ...
      Effect.fnUntraced(function* (st) {
        ...
        if (st._tag === "ShellThenRun" && st.shell.id === id) {
          return yield* CssltdRunner.commit(startRun(st.run.work, st.run.done), Effect.void)
        }
      ...
      Effect.fnUntraced(function* (st) {
        switch (st._tag) {
          ...
          case "Idle": {
            ...
            return yield* CssltdRunner.commit(startRun(work, done), awaitDone(done))
          }
        }
      }),
    ).pipe(Effect.flatten)
