...
import type * as ModelsDev from "@cssltdcode/core/models-dev"
import { iife } from "@/util/iife"
import { cssltdProviderOptions } from "@/cssltdcode/provider-options"
import { isLing } from "@/cssltdcode/model-match" // cssltdcode_change
import { reasoningSummary } from "@/cssltdcode/provider/reasoning-summary" // cssltdcode_change
...
function sdkKey(npm: string): string | undefined {
  switch (npm) {
    ...
    case "@openrouter/ai-sdk-provider":
      return "openrouter"
    case "@cssltdcode/cssltd-gateway": // cssltdcode_change
      return "openrouter"
...
function normalizeMessages(
  ...
  // cssltdcode_change start - only filter for Claude models on Bedrock, not all Bedrock models
...
export function temperature(model: Provider.Model) {
  ...
  if (isLing(model.api.id)) return 0.3 // cssltdcode_change
  return undefined
}
...
export function topP(model: Provider.Model) {
  ...
  if (isLing(model.api.id)) return 0.95 // cssltdcode_change
  return undefined
}
...
export function topK(model: Provider.Model) {
  ...
  if (isLing(model.api.id)) return 20 // cssltdcode_change
  return undefined
}
...
// cssltdcode_change start - fable and sonnet-5 models are adaptive thinking models like opus-4.7/4.8
...
function anthropicAdaptiveEfforts(apiId: string): string[] | null {
  // cssltdcode_change start - treat opus-4.8 and fable like opus-4.7
...
function anthropicOmitsThinking(apiId: string) {
  return anthropicOpus47OrLater(apiId) || anthropicClaude5(apiId) // cssltdcode_change - include Cssltd's fable/sonnet-5 aliases
}
...
function googleThinkingVariants(model: Provider.Model): Record<string, Record<string, any>> {
  ...
  if (id.includes("gemma")) return {} // cssltdcode_change
  if (id.includes("2.5")) {
    return {
...
export function variants(model: Provider.Model): Record<string, Record<string, any>> {
  ...
  if (
    ["@cssltdcode/cssltd-gateway", "@ai-sdk/openai-compatible"].includes(model.api.npm) &&
    model.variants &&
  ...
  if (
    ...
    // id.includes("minimax") || // cssltdcode_change
    // id.includes("glm") || // cssltdcode_change
    // id.includes("kimi") || // cssltdcode_change
  ...
  if (id.includes("grok") && id.includes("grok-3-mini")) {
    if (model.api.npm === "@openrouter/ai-sdk-provider" || model.api.npm === "@cssltdcode/cssltd-gateway") {
      // cssltdcode_change - add Cssltd Gateway support
      return {
  ...
  switch (model.api.npm) {
    case "@cssltdcode/cssltd-gateway": // cssltdcode_change
      ...
      if (
        ...
        !id.includes("claude") &&
        !model.id.includes("mercury") // cssltdcode_change
      )
        return {}
      return Object.fromEntries(
        OPENAI_EFFORTS.map((effort) => [effort, { reasoning: { effort } }]), // cssltdcode_change
      )

    case "@openrouter/ai-sdk-provider":
      // cssltdcode_change start - preserve Cssltd thinking toggles for supported OpenRouter models
      if (id.includes("glm") || id.includes("kimi") || id.includes("qwen") || id.includes("minimax")) {
        return {
          instant: { reasoning: { enabled: false } },
          thinking: { reasoning: { enabled: true } },
      ...
      // cssltdcode_change end
      // cssltdcode_change start - Mercury exposes the full OpenRouter effort set
    ...
    case "@ai-sdk/openai": {
      ...
      return Object.fromEntries(
        efforts.map((effort) => [
          ...
          {
            reasoningEffort: effort,
            reasoningSummary: reasoningSummary(model), // cssltdcode_change
            include: INCLUDE_ENCRYPTED_REASONING,
          },
        ]),
      )
    ...
    case "@ai-sdk/google-vertex/anthropic":
      ...
      // cssltdcode_change start - MiniMax M-series toggles thinking on/off rather than exposing effort levels
    ...
    case "@ai-sdk/mistral":
      ...
      // https://docs.mistral.ai/studio-api/conversations/reasoning // cssltdcode_change
      if (!model.capabilities.reasoning) return {}
...
export function options(input: {
  ...
  if (
    ...
    input.model.api.npm === "@cssltdcode/cssltd-gateway" // cssltdcode_change
  ) {
    result["usage"] = {
      include: true,
    }
  ...
  if (
    ...
    (input.model.providerID === "cssltdcode" && ["kimi-k2-thinking", "glm-4.6"].includes(input.model.api.id))
  ) {
  ...
  if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
    if (!input.model.api.id.includes("gpt-5-pro")) {
      ...
      if (
        ...
        input.model.api.npm === "@ai-sdk/github-copilot" || // cssltdcode_change
        input.model.api.npm === "@openrouter/ai-sdk-provider" || // cssltdcode_change
        input.model.api.npm === "@cssltdcode/cssltd-gateway" || // cssltdcode_change
        input.model.api.npm === "@ai-sdk/amazon-bedrock/mantle"
      ) {
        result["reasoningSummary"] = reasoningSummary(input.model) // cssltdcode_change
    ...
    if (
      // cssltdcode_change start - gate textVerbosity to Responses-API providers
      (input.model.api.npm === "@ai-sdk/openai" ||
        input.model.api.npm === "@ai-sdk/azure" ||
        input.model.api.npm === "@ai-sdk/amazon-bedrock/mantle" ||
        input.model.api.npm === "@ai-sdk/github-copilot" ||
        input.model.api.npm === "@openrouter/ai-sdk-provider" ||
        input.model.api.npm === "@cssltdcode/cssltd-gateway") &&
      // cssltdcode_change end
    ...
    if (input.model.providerID.startsWith("cssltdcode")) {
      result["promptCacheKey"] = input.sessionID
      result["include"] = INCLUDE_ENCRYPTED_REASONING
      result["reasoningSummary"] = "auto"
    }
...
export function smallOptions(model: Provider.Model) {
  ...
  if (
    ...
    model.api.npm === "@cssltdcode/cssltd-gateway" // cssltdcode_change
  ) {
    if (model.providerID === "openrouter" && small.reasoning?.effort === "low") {
      return { reasoning: { effort: "none" } }
    }
    if (!model.capabilities.reasoning) return {} // cssltdcode_change - omit unsupported reasoning options
    return { reasoning: { enabled: true } } // cssltdcode_change - use the model's supported default effort
  }
...
export function providerOptions(model: Provider.Model, options: { [x: string]: any }) {
  ...
  // cssltdcode_change start
  if (model.api.npm === "@cssltdcode/cssltd-gateway") {
    return cssltdProviderOptions(options)
  }
  // cssltdcode_change end
  ...
  */
  ...
  if (model.providerID === "google" || model.api.id.includes("gemini")) {
    ...
    const sanitizeGemini = (obj: any): any => {
      ...
      // cssltdcode_change start - Gemini rejects required entries without matching properties
