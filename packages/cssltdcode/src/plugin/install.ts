...
export type PatchDeps = {
  ...
  files: (dir: string, name: "cssltdcode" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".cssltd") // cssltdcode_change
}

function patchName(kind: Kind): "cssltdcode" | "tui" {
  if (kind === "server") return "cssltdcode"
  return "tui"
}
