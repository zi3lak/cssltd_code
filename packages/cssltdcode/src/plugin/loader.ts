...
export namespace PluginLoader {
  ...
  > {
    ...
    // npm plugins can declare which cssltdcode versions they support; file plugins are treated
  ...
  async function attempt<R>(
    ...
    if (isIndexingPlugin(candidate.plan.spec)) return { retry: false } // cssltdcode_change
    if (isAtomicChatPlugin(candidate.plan.spec)) return { retry: false } // cssltdcode_change
