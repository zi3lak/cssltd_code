...
import { refreshCodexAuth } from "@/cssltdcode/provider/codex-refresh" // cssltdcode_change
import { OpenAIWebSocketPool } from "./ws-pool"

const log = Log.create({ service: "plugin.codex" }) // cssltdcode_change
...
const ALLOWED_MODELS = new Set([
  ...
  "gpt-5.4",
  "gpt-5.4-mini",
  // cssltdcode_change start - additional codex models supported by Cssltd
  "gpt-5.1-codex",
  "gpt-5.1-codex-max",
  "gpt-5.1-codex-mini",
  "gpt-5.2-codex",
...
async function refreshAccessToken(refreshToken: string, issuer = ISSUER, signal?: AbortSignal): Promise<TokenResponse> {
  const response = await fetch(`${issuer}/oauth/token`, {
    method: "POST",
    signal,
    headers: { "Content-Type": "application/x-www-form-urlencoded", "User-Agent": `cssltd/${InstallationVersion}` },
    // cssltdcode_change end
    body: new URLSearchParams({
...
<html>
  <head>
    <!-- cssltdcode_change start -->
    <title>Cssltd - Codex Authorization Successful</title>
    <!-- cssltdcode_change end -->
    <style>
      body {
        font-family:
  ...
  <body>
    <div class="container">
      ...
      <!-- cssltdcode_change start -->
      <p>You can close this window and return to Cssltd.</p>
      <!-- cssltdcode_change end -->
    </div>
    <script>
...
<html>
  <head>
    <!-- cssltdcode_change start -->
    <title>Cssltd - Codex Authorization Failed</title>
    <!-- cssltdcode_change end -->
    <style>
      body {
        font-family:
...
export async function CodexAuthPlugin(input: PluginInput, options: CodexAuthPluginOptions = {}): Promise<Hooks> {
  ...
  return {
        ...
        return Object.fromEntries(
          Object.entries(provider.models)
            .filter(([, model]) => {
              ...
              if (DISALLOWED_MODELS.has(model.api.id)) return false // cssltdcode_change
    ...
    "chat.headers": async (input, output) => {
      ...
      output.headers.originator = "cssltdcode"
      output.headers["User-Agent"] = `cssltdcode/${InstallationVersion} (${os.platform()} ${os.release()}; ${os.arch()})`
