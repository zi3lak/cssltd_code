...
export const DEPRECATED_PLUGIN_PACKAGES = ["cssltdcode-openai-codex-auth", "cssltdcode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, cssltdcodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(cssltdcodeVersion) || semver.major(cssltdcodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.cssltdcode
  if (typeof range !== "string") return
  if (!semver.satisfies(cssltdcodeVersion, range)) {
    throw new Error(`Plugin requires cssltdcode ${range} but running ${cssltdcodeVersion}`)
  }
}
