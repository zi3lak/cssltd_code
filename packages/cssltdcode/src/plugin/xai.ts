...
function authHeaders() {
  return {
    ...
    "User-Agent": `cssltdcode/${InstallationVersion}`, // cssltdcode_change
  }
}
...
export function buildAuthorizeUrl(
  ...
  // clients. `referrer=cssltdcode` lets xAI attribute cssltdcode-originated
  ...
  const params = new URLSearchParams({
    ...
    nonce,
    plan: "generic",
    referrer: "cssltdcode", // cssltdcode_change
  })
...
<html>
  <head>
    <title>Cssltdcode - xAI Authorization Successful</title>
    <style>
      body {
        font-family:
          system-ui,
  ...
  <body>
    <div class="container">
      ...
      <p>You can close this window and return to Cssltdcode.</p>
    </div>
    <script>
...
<html>
  <head>
    <title>Cssltdcode - xAI Authorization Failed</title>
    <style>
      body {
        font-family:
          system-ui,
...
async function startOAuthServer(): Promise<{ port: number; redirectUri: string }> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    server.listen(OAUTH_PORT, OAUTH_HOST, () => {
      ...
      // cssltdcode_change start
      // behavior and crash the entire Cssltdcode process. Matches the silent-
      // cssltdcode_change end
      ...
      async loader(getAuth) {
        ...
        return {
          ...
          async fetch(requestInput: RequestInfo | URL, init?: RequestInit) {
            ...
            headers.set("User-Agent", `cssltdcode/${InstallationVersion}`) // cssltdcode_change
