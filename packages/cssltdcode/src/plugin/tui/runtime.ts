...
import { errorData, errorMessage } from "@cssltdcode/tui/util/error"
...
import { hasTheme, upsertTheme } from "@cssltdcode/tui/context/theme"
...
import type { HostPluginApi, HostSlots } from "@cssltdcode/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@cssltdcode/core/v1/config/plugin"
import { createCommandShim } from "@cssltdcode/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@cssltdcode/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    // cssltdcode_change start - install local themes into supported Cssltd config directories
    const base = path.basename(source_dir)
    const local_dir =
      base === ".cssltd" || base === ".cssltdcode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".cssltd", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".cssltd", "tui.json"), // cssltdcode_change
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.CSSLTD_PURE ? [] : pluginOrigins
    if (Flag.CSSLTD_PURE && pluginOrigins.length) {
    }
