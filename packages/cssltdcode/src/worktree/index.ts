...
import { path } from "@cssltdcode/core/effect/layer-node-platform"
...
import { WorktreeCleanup } from "@/cssltdcode/worktree-cleanup" // cssltdcode_change
...
export class Service extends Context.Service<Service, Interface>()("@cssltdcode/Worktree") {}
  ...
  Effect.gen(function* () {
    ...
    const candidate = Effect.fn("Worktree.candidate")(function* (input: {
      ...
      for (const attempt of Array.from({ length: MAX_NAME_ATTEMPTS }, (_, i) => i)) {
        ...
        const branch = input.detached ? undefined : `cssltdcode/${name}`
    ...
    // cssltdcode_change start - use Cssltd cleanup helper for slow Windows handle release
