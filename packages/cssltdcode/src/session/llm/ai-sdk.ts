import { FinishReason, LLMEvent, ProviderMetadata, ToolResultValue } from "@cssltdcode/llm"
import { Effect, Schema } from "effect"
import { type streamText } from "ai"
import { errorMessage } from "@/util/error"
import { CssltdRoutedModel } from "@/cssltdcode/session/routed-model" // cssltdcode_change
...
// Move this extraction into @cssltdcode/llm when Copilot is handled by the native runtime.
...
function usage(value: unknown) {
  ...
  const item = value as {
    ...
    raw?: Record<string, unknown> // cssltdcode_change - preserve provider billing details
  }
  const entries = Object.entries({
    ...
    providerMetadata: item.raw ? { aiSdk: item.raw } : undefined, // cssltdcode_change - retain Cssltd billing details
      ...
      return Effect.sync(() => {
        ...
        return [
          LLMEvent.stepFinish({
            ...
            providerMetadata: CssltdRoutedModel.write(metadata, event.response?.modelId), // cssltdcode_change
          }),
        ]
      })
