...
import type { LLMClientShape } from "@cssltdcode/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("cssltdcode"))
    return { type: "unsupported", reason: "provider is not openai, cssltdcode, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @cssltdcode/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains cssltdcode-owned. The native runtime only adapts
      // the @cssltdcode/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
