...
import { Module } from "@cssltdcode/core/util/module"
import { spawn } from "./launch"
import { Npm } from "@cssltdcode/core/npm"
import { TsCheck } from "../cssltdcode/ts-check" // cssltdcode_change
import type { RuntimeFlags } from "@/effect/runtime-flags"

const log = Log.create({ service: "lsp.server" }) // cssltdcode_change
...
// cssltdcode_change start - tsgo native LSP or lightweight diagnostic client
// When CSSLTD_EXPERIMENTAL_LSP_TOOL is enabled, spawn tsgo --lsp --stdio as a
...
export const Typescript: Info = {
  ...
  async spawn(root) {
    if (!Flag.CSSLTD_EXPERIMENTAL_LSP_TOOL) return undefined
...
export const Clangd: Info = {
  ...
  async spawn(root, _ctx, flags) {
    ...
    // cssltdcode_change start - reject release metadata before it becomes an executable path
    ...
    // cssltdcode_change start - do not use remote metadata as a local path
...
export const JDTLS: Info = {
  ...
  async spawn(root, _ctx, flags) {
    ...
    const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "cssltdcode-jdtls-data"))
    return {
      process: spawn(
        java,
        [
          "-Djava.import.generatesMetadataFilesAtProjectRoot=false", // cssltdcode_change
          "-jar",
          launcherJar,
          "-configuration",
          configFile,
...
export const LuaLS: Info = {
  ...
  async spawn(root, _ctx, flags) {
    ...
    if (!bin) {
      ...
      // cssltdcode_change start - use a fixed local archive name
