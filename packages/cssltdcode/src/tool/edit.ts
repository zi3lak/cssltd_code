...
import * as Encoding from "../cssltdcode/encoding" // cssltdcode_change

const MAX_DIFF_CONTENT = 500_000 // cssltdcode_change
    ...
    return {
      ...
      execute: (params: Schema.Schema.Type<typeof Parameters>, ctx: Tool.Context) =>
        Effect.gen(function* () {
          ...
          let contentOld = ""
          let contentNew = ""
          let cachedFilediff: Snapshot.FileDiff | undefined // cssltdcode_change
          yield* lock(filePath).withPermits(1)(
            Effect.gen(function* () {
              if (params.oldString === "") {
                ...
                cachedFilediff = buildFileDiff(filePath, contentOld, contentNew) // cssltdcode_change
                yield* ctx.ask({
                  permission: "edit",
                  ...
                  metadata: {
                    filepath: filePath,
                    diff,
                    filediff: cachedFilediff, // cssltdcode_change
                  },
                })
                yield* EncodedIO.write(afs, filePath, Bom.join(contentNew, desiredBom), Encoding.DEFAULT) // cssltdcode_change - encoding-aware write (mkdirs) replaces afs.writeWithDirs
              ...
              yield* EncodedIO.write(afs, filePath, Bom.join(contentNew, desiredBom), source.encoding) // cssltdcode_change - encoding-aware write replaces afs.writeWithDirs
              if (yield* format.file(filePath)) {
          ...
          const filediff: Snapshot.FileDiff = cachedFilediff ?? buildFileDiff(filePath, contentOld, contentNew) // cssltdcode_change
