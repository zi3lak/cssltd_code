...
import { Icon, type IconProps } from "./icon" // cssltdcode_change: added Icon
...
export interface BasicToolProps {
  ...
  forceOpen?: boolean
  defer?: boolean
  retainDetails?: boolean // cssltdcode_change
  hasDetails?: boolean // cssltdcode_change
  locked?: boolean
  animated?: boolean
  allowPendingToggle?: boolean // cssltdcode_change
...
export function BasicTool(props: BasicToolProps) {
  ...
  const hasDetails = () => props.hasDetails ?? !!hasChildren() // cssltdcode_change
  ...
  createEffect(
    on(
      ...
      (value) => {
        ...
        if (!value) {
          cancel()
          if (!props.retainDetails) setState("ready", false) // cssltdcode_change
          return
        }
  ...
  const handleOpenChange = (value: boolean) => {
    if (pending() && !props.allowPendingToggle) return // cssltdcode_change
    if (props.hideDetails) return // cssltdcode_change
    if (props.locked && !value) return
    setOpen(value)
    props.onOpenChange?.(value) // cssltdcode_change
  }
