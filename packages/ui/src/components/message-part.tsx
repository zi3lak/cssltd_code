...
import { getDirectory as _getDirectory, getFilename } from "@cssltdcode/core/util/path"
...
export interface MessageProps {
  ...
  queued?: boolean // cssltdcode_change
      ...
      <Match when={props.message.role === "user" && props.message}>
        {(userMessage) => (
          <UserMessageDisplay
            ...
            queued={props.queued} // cssltdcode_change
          />
        )}
      </Match>
            ...
            {(file) => {
              ...
              return (
                <div
                  ...
                  data-queued={props.queued ? "" : undefined} // cssltdcode_change
      ...
      <Show when={text()}>
        <>
          <div data-slot="user-message-body">
            <div data-slot="user-message-text" dir="auto" data-queued={props.queued ? "" : undefined}>{/* cssltdcode_change */}
...
PART_MAPPING["text"] = function TextPartDisplay(props) {
  ...
  const isLastTextPart = createMemo(() => {
    const last = (data.store.part?.[props.message.id] ?? [])
      .filter((item): item is TextPart => item?.type === "text" && !!item.text?.trim() && !item.synthetic) // cssltdcode_change
      .at(-1)
    return last?.id === part().id
  })
  const showCopy = createMemo(() => {
    if (part().synthetic) return false // cssltdcode_change
  ...
  return (
    <Show when={text() && showSyntheticPart() /* cssltdcode_change */}>
