...
import { stableBlocks } from "../cssltdcode/markdown-stable-blocks" // cssltdcode_change
...
export function stream(text: string, live: boolean) {
  ...
  const candidate = tokens.findLast((token) => token.type !== "space") // cssltdcode_change
  const blocks = candidate && !open(candidate.raw) ? stableBlocks(tokens, heal) : undefined // cssltdcode_change
  if (blocks) return blocks // cssltdcode_change
