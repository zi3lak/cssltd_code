...
// The "Cssltd" Pierre/Shiki theme used by every diff review surface (Code / Diff /
// File / SessionReview) and by markdown code highlighting. Pierre resolves the
// theme by name when a worker pool initializes (resolveThemes(["Cssltd"])) or when
// getSharedHighlighter() attaches it; if the name was never registered it throws
// "resolveTheme: No valid loader for Cssltd".
//
...
// diff component imports the worker factory, which calls ensureCssltdDiffTheme()
...
// without markdown (e.g. cssltd-console) to fire a racy `void import(...)` purely to
// register the theme. Keeping it here, free of katex/marked, lets those consumers
// stay light while removing the race entirely.
//
// Upstream owns the equivalent registerCustomTheme("CssltdCode", …) block inline in
// context/marked.tsx. Do not restore that inline block on upstream merges — route
// the registration through ensureCssltdDiffTheme() instead.

export const CSSLTD_DIFF_THEME = "Cssltd"
...
export function ensureCssltdDiffTheme(): void {
  if (RegisteredCustomThemes.has(CSSLTD_DIFF_THEME)) return

  registerCustomTheme(CSSLTD_DIFF_THEME, () => {
    return Promise.resolve({
      name: CSSLTD_DIFF_THEME,
      colors: {
