...
import { createCssltd } from "@cssltdcode/sdk"
...
`)
  ...
  const cssltdcode = await createCssltd({ port: 0 })
  ...
  try {
    ...
    const session = await cssltdcode.client.session.create()
    const result = await cssltdcode.client.session
      .prompt({
