...
const repo = process.env.GH_REPO ?? "Cssltd-Org/cssltdcode"
const bot = ["actions-user", "github-actions[bot]", "cssltdcode", "cssltdcode-agent[bot]"]
const team = [
...
const order = ["Core", "TUI", "SDK", "Extensions"] as const // cssltdcode_change
const sections = {
  core: "Core",
  tui: "TUI",
  sdk: "SDK",
...
function section(areas: Set<string>) {
  const priority = ["core", "tui", "sdk", "plugin", "extensions/vscode", "github"] // cssltdcode_change
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/cssltdcode packages/sdk packages/plugin packages/extensions github`.text() // cssltdcode_change
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/cssltdcode/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/cssltdcode/")) areas.add("core")
