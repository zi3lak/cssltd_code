...
const model = "cssltdcode/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/cssltdcode")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with cssltdcode...`)
  ...
  try {
    await $`cssltdcode run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  cssltdcode failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with cssltdcode")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with cssltdcode...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/cssltdcode`.",
